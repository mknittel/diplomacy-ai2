class Province:
    def print_data(self):
        for loc in self.fleet_neighbors and loc != self.name:
            print self.name, "can reach", loc, "by fleet."
        for loc in self.army_neighbors and loc != self.name:
            print self.name, "can reach", loc, "by army."

        if not self.fleet_neighbors and loc != self.name:
            print self.name, "has no neighbors it can reach by fleet."
        
        if not self.army_neighbors and loc != self.name:
...
class SeaProvince(Province):
    def add_sea_neighbor(self, loc):
        if loc not in self.fleet_neighbors and loc != self.name:
    ...
    def add_land_neighbor(self, loc):
        ...
        if loc not in self.army_neighbors and loc != self.name:
...
class LandProvince(Province):
    ...
    def landlocked(self):
        return len(self.fleet_neighbors) == 0
